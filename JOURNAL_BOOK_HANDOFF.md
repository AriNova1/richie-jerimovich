...
18. **Un-split `<ul>`/`<pre>` blocks can blow past 100% fill.** The pagination engine (§6.1) treats list and code blocks as atomic (unlike paragraphs, they can't be word-split at an arbitrary point) and will place one "best effort" even when oversized. Found at CP3: one real entry's 193-word bulleted list blew a single page to 147.7% fill (genuine clipping) before a dedicated list/code-block splitter was added, plus CSS for lists/code blocks inside entry bodies, plus excluding `<pre>` content from the word-jitter pass (jittering would wreck monospace alignment). If a future entry with a large list or code block starts overflowing, this is the mechanism — the fix already exists in the pagination script, so first confirm the splitter is actually being invoked for that block type before assuming it's a new bug.
